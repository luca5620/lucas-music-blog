...
import NewRoomForm from "@/components/aux-wars/NewRoomForm";
import BackLink from "@/components/ui/BackLink";
// LANGUAGES: every word we wrote comes from messages/<locale>.json.
import { getTranslations } from "next-intl/server";

export const metadata: Metadata = {
  title: "Host an Aux War",
};

/** /aux-wars/new — host a room (signed-in only; middleware also guards). */
export default async function NewAuxBattlePage() {
  const user = await getUser();
  if (!user) redirect("/login?next=/aux-wars/new");
