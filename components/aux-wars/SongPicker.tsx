 ...
 * server (/api/aux-wars/songs says which are on; the tab then
...
import { sourceTag } from "@/components/aux-wars/SongEmbed";
...
export default function SongPicker({ onPick, tone }: Props) {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`/api/aux-wars/songs?q=&source=spotify`)
    ...
    async (q: string, src: Source) => {
      ...
      try {
        const res = await fetch(
          `/api/aux-wars/songs?q=${encodeURIComponent(q)}&source=${src}`
        );
  ...
  async function readLink() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/aux-wars/songs", {
        method: "POST",
