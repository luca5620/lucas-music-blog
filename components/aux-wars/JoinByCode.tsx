...
export default function JoinByCode() {
  ...
  function go(e: React.FormEvent) {
    ...
    if (!raw) return;
    // Accept the full room link or just the slug — and still read a
    // link from before the rename, since someone's friend may have
    // sent them an /aux-battles/ one yesterday.
    const m = raw.match(/aux-(?:wars|battles)\/([a-z0-9-]+)/i);
    const slug = (m ? m[1] : raw).toLowerCase().replace(/[^a-z0-9-]/g, "");
    if (!slug) return;
    router.push(`/aux-wars/${slug}`);
  }
