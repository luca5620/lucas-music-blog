...
/**
 * DeleteRoomButton — the host tears an Aux War room down (DELETE
 * /api/aux-wars/[roomId]; bracket, votes and chat cascade). Inline
...
export default function DeleteRoomButton({ roomId, name }: { roomId: string; name: string }) {
  ...
  async function remove() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/aux-wars/${roomId}`, { method: "DELETE" });
      if (!res.ok) {
