...
/**
 * AuxRoom — the live client half of an Aux War page. Three looks,
...
import { auxPlayerCap } from "@/lib/aux-wars/limits";
...
import PlayerChip, { AuxAvatar, WinsTag } from "@/components/aux-wars/PlayerChip";
import SongPicker from "@/components/aux-wars/SongPicker";
import TopicPicker from "@/components/aux-wars/TopicPicker";
import SongEmbed, { sourceTag } from "@/components/aux-wars/SongEmbed";
import Bracket from "@/components/aux-wars/Bracket";
import WinnerBurst from "@/components/aux-wars/WinnerBurst";
import AuxChatDock from "@/components/aux-wars/AuxChatDock";
...
export default function AuxRoom({
  ...
  const resync = useCallback(async () => {
    try {
      const res = await fetch(`/api/aux-wars/${room.id}/state`, { cache: "no-store" });
      if (!res.ok) return;
  ...
  const join = (role: "player" | "viewer") =>
    act(`join-${role}`, () => post(`/api/aux-wars/${room.id}/members`, { role }));
  const leave = () =>
    act("leave", async () => {
      const res = await fetch(`/api/aux-wars/${room.id}/members`, { method: "DELETE" });
      if (!res.ok) throw new Error(((await res.json().catch(() => ({}))) as { error?: string }).error ?? "Couldn't leave.");
    });
  const start = () => act("start", () => post(`/api/aux-wars/${room.id}/start`));
  const end = () => {
    if (!window.confirm(t("endConfirm"))) return;
    void act("end", () => post(`/api/aux-wars/${room.id}/end`));
  };
  const vote = (side: "a" | "b") =>
    act(`vote-${side}`, async () => {
      setMyVote(side);
  ...
  const pick = async (song: AuxSong) => {
    await post(`/api/aux-wars/${room.id}/pick`, { song });
  };
  const share = () => {
    hapticTap();
    void shareLink(room.name, `${window.location.origin}/aux-wars/${room.slug}`);
  };
  const copyCode = () => {
    if (!code) return;
    hapticTap();
