...
import type { AuxInvitable } from "@/lib/db/aux-wars";
...
export default function InviteFriends({ roomId }: { roomId: string }) {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch(`/api/aux-wars/${roomId}/invite`, { cache: "no-store" });
  ...
  async function invite(userId: string) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/aux-wars/${roomId}/invite`, {
        method: "POST",
